"""Configurable number entities for Ambient Music (volume, fade durations, etc.)."""
...
class AmbientMusicNumber(NumberEntity, RestoreEntity):
    """Restorable number entity created from a NUMBER_ENTITIES tuple."""
    ...
    def __init__(self, key: str, min_val: float, max_val: float, step: float) -> None:
        """
        Initialise from a NUMBER_ENTITIES tuple.

        :param key: Translation key and unique-id suffix.
    ...
    async def async_set_native_value(self, value: float) -> None:
        """Set the number value, writing state only when it actually changes."""
        v = float(value)
...
async def async_setup_entry(
    ...
    """Create one number entity per NUMBER_ENTITIES entry."""
    entities = [
